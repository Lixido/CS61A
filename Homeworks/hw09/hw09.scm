(define (how-many-dots s)
    (if (not (pair? s))
        0
        (+ (if (not (or (nil? (cdr s)) (pair? (cdr s))))
                1
                0)
            (how-many-dots (car s))
            (how-many-dots (cdr s)))))
...
(define (derive-exp exp var)
    (define x (base exp)) (define n (exponent exp))
    (if (number? n)
        ;make-product only takes two arguments, so x'*n*x^n-1 is devided
        ((define temp (make-product n (derive x var)))
        (make-product temp (make-exp x n-1)))
        'toocomplicated))