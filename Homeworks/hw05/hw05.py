...
def balanced(m):
    ...
    left_end = end(left)
    right_end = end(right)
    return total_weight(left_end) * length(left) == total_weight(right_end) * length(right)\
...
class Account:
    ...
    def time_to_retire(self, amount):
        ...
        time, balance = 0, self.balance
        while balance < amount:
            time, balance = time + 1, balance + self.interest * balance
        return time
...
def make_withdraw(balance, password):
    ...
    error_times = 0
    error_pw = []
    def withdraw_pw(amount, input_pw):
        nonlocal balance, error_times
        if error_times < 3:
...
def make_joint(withdraw, old_password, new_password):
    ...
    if type(result) is not str:
        def withdraw_joint(amount, input_pw):
            if input_pw == new_password:
                input_pw = old_password  # Such a genious method! Enlightened by the internet
            return withdraw(amount, input_pw)
...
def sub_interval(x, y):
    ...
    and any value in y."""
    negative_y = interval(-lower_bound(y), -upper_bound(y))
...
def multiple_references_explanation():
    return """When multiplying uncertain numbers, interval will be extended, leading to a larger error bound. 
    In par1, such a process exists.
    In par2, it is avoided since all multiplication (division) is done with a certain number -- one.
    """
...
def quadratic(x, a, b, c):
    ...
    '0 to 10'
    """
    assert a != 0, "quadratic term coefficient cannot be 0"
    t = - b / (2 * a)
    f = lambda x: a * x * x + b * x + c
    y_lower, y_upper, y_t = f(lower_bound(x)), f(upper_bound(x)), f(t)
    if lower_bound(x) <= t and upper_bound(x) >= t:
        return interval(min(y_lower, y_upper, y_t), max(y_lower, y_upper, y_t))   
    else:
        return interval(min(y_lower, y_upper), max(y_lower, y_upper))


# Too complicated so i will just copy the official solution
