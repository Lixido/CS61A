...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        distance = 0
        def nearest_place(self, hive, where):
            nonlocal distance
            if where == hive:
                return None
            elif random_or_none(where.bees) and self.min_range <= distance <= self.max_range:
                return random_or_none(where.bees)
            else:
                distance += 1
                return nearest_place(self, hive, where.entrance)
        return nearest_place(self, hive, self.place)
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0:
            ...
            # same as bees = list(self.place.bees)
            '''
            Hint: As the Python tutorial suggests, 
            "If you need to modify the sequence you are iterating over 
            while inside the loop (for example to duplicate selected items), 
            it is recommended that you first make a copy." 
            You can copy a list by calling the list constructor or 
...
# BEGIN Problem 8
class WallAnt(Ant):
    """WallAnt has a large armor value and does nothing else."""
    name = 'Wall'
    food_cost = 4
    implemented = True
...
class BodyguardAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 9
        if self.ant:
            self.ant.action(colony)  # let the protected ant do its action
        # END Problem 9
