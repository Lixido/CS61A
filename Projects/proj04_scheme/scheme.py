...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(operator, MacroProcedure):  # PROBLEM 21 UPDATE
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        value = scheme_eval(expressions.second.first, env)  # expr.second.second = nil
...
def make_let_frame(bindings, env):
    ...
    while not bindings is nil:
        ...
        name, value = binding.first, scheme_eval(binding.second.first, env)
